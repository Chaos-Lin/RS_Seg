...
from ..subnet import ConvBNReLU,ASPPModule,SeparableConvBNReLU
...
class DeepLabV3P(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.data_format == 'NCHW':
            ori_shape = torch.shape(x)[2:]
        else:
            ori_shape = torch.shape(x)[1:3]
        return [
            F.interpolate(
                logit,
                ori_shape,
...
class Decoder(nn.Module):
    ...
    def forward(self, x, low_level_feat):
        ...
        # if self.data_format == 'NCHW':
        low_level_shape = torch.shape(low_level_feat)[-2:]
        axis = 1
        # else:
        #     low_level_shape = torch.shape(low_level_feat)[1:3]
        #     axis = -1
        x = F.interpolate(
            x,
            low_level_shape,
