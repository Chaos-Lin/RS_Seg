...
from ..subnet import ConvBNReLU,AuxLayer
...
class ENCNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.fcn_head = AuxLayer(self.backbone.feat_channels[2],
...
class Encoding(nn.Module):
    ...
    def scaled_l2(self, x, codewords, scale):
        num_codes, channels = torch.shape(codewords)
        reshaped_scale = scale.reshape([1, 1, num_codes])
        expanded_x = torch.tile(x.unsqueeze(2), [1, 1, num_codes, 1])
        reshaped_codewords = codewords.reshape([1, 1, num_codes, channels])

        scaled_l2_norm = torch.multiply(
            reshaped_scale,
...
class EncModule(nn.Module):
    ...
    def forward(self, x):
        ...
        batch_size, _, _, _ = torch.shape(x)
