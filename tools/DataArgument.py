...
def creat_dataset(image_sets,img_w,img_h,image_num = 5000, mode = 'normal'):
    ...
    for i in tqdm(range(len(image_sets))):
        count = 0
        src_img = cv2.imread('../Dataset/RM/JPEGImages/' + image_sets[i])  # 3 channels
        # label_img = cv2.imread('./Training Set/Target maps/' + image_sets[i].replace('tiff','tif'))  # 3 channels
        label_img_gray = cv2.imread('../Dataset/RM/SegmentationClass/' + image_sets[i].replace('tiff', 'tif'),
                                    cv2.IMREAD_GRAYSCALE)  # single channel
        # src_img = test_img.resize((img_w, img_h))
        ...
        while count < image_each:
            ...
            cv2.imwrite(('../Dataset/RMed/JPEGImages/%d_image.png' % g_count),src_roi)
            cv2.imwrite(('../Dataset/RMed/SegmentationClass/%d_image.png' % g_count),label_roi_gray)
            count += 1 
            g_count += 1
...
if __name__ == '__main__':
    img_w = 256  
    img_h = 256  
    src_data_path = '../Dataset/RM/JPEGImages'
    label_data_path = '../Dataset/RM/SegmentationClass'
